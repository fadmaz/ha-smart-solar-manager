...
def build_recommendation(
    ...
    weighted_signal = (
        ...
        + weights["self_consumption"] * max(solar_surplus_w, 0.0)
        + weights["battery_health"] * max(battery_soc - battery_min_soc, 0.0)
        + weights["grid"] * max(forecast_next_hour_w, 0.0)
    )

    estimated_savings = 0.0
    if mode == "run_flexible_loads" and solar_surplus_w > 0:
        estimated_savings = round((solar_surplus_w / 1000.0) * grid_price, 4)
